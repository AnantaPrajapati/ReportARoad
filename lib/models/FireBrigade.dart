...
class FireBrigade extends StatefulWidget {
  @override
  State<FireBrigade> createState() => _ViewAddressPageState();
}

class _ViewAddressPageState extends State<FireBrigade> {
  ...
  Future<void> _fetchNearbyPoliceStations() async {
    Position position = await Geolocator.getCurrentPosition(desiredAccuracy: LocationAccuracy.high);
    final url =
        'https://map-places.p.rapidapi.com/nearbysearch/json?location=${position.latitude}%2C${position.longitude}&radius=1500&keyword=FireBrigade&type=FireBrigade';
